...
function validatePatchPaths(manifestPath, packageRoot, patchDeclaration) {
  const patches = Array.isArray(patchDeclaration)
    ? patchDeclaration
    : [patchDeclaration];
  if (patches.length === 0) {
    die(`${manifestPath} must declare at least one dsh.bundle.patch path`);
  }

  for (const patch of patches) {
    if (typeof patch !== "string" || patch.length === 0) {
      die(`${manifestPath} dsh.bundle.patch entries must be non-empty strings`);
    }
    if (!patch.startsWith("./") || patch.includes("\\")) {
      die(
        `${manifestPath} dsh.bundle.patch entries must be relative './...' paths`,
      );
    }
    ...
    if (relativePatch.startsWith("..") || path.isAbsolute(relativePatch)) {
      die(
        `${manifestPath} dsh.bundle.patch escapes the package root: ${patch}`,
      );
    }
    if (!fs.existsSync(patchPath) || !fs.statSync(patchPath).isFile()) {
      die(`${manifestPath} dsh.bundle.patch does not exist: ${patch}`);
    }
  }
}
...
function bundleMetadata(packageRoot, root) {
  ...
  if (typeof bundle.patch !== "string" && !Array.isArray(bundle.patch)) {
    die(
      `${manifestPath} must declare dsh.bundle.patch as a non-empty string or array`,
    );
  }
  ...
  validatePatchPaths(manifestPath, packageRoot, bundle.patch);
...
function readManifest(file) {
  ...
  for (const bundle of manifest.bundles) {
    if (
      ...
      (typeof bundle.patch !== "string" && !Array.isArray(bundle.patch)) ||
