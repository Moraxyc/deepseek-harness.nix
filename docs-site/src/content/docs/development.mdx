...
`packages` provides `dsh`, `dsh-desktop`, `dsh-desktop-official`,
`dsh-desktop-unofficial`, `dsh-kernel`, and `dsh-workspace`; `dsh-desktop` is an
alias and currently resolves to `dsh-desktop-unofficial`.
`bundles.*` and `presets.*` are exposed through `legacyPackages`, so refs such
as `.#bundles.tui` and `.#presets.tui` work directly. `bundleCompositions.*` is
exposed the same way: the default `dsh` composition with one bundle layered on
top, skipped for bundles the default composition already carries. It exists to
validate bundle layering, not as a package to install. See
...
Keep release-specific upstream migration notes in the
[Migration Guide](../migration/) and its Chinese counterpart. Add a versioned
entry when an upstream update changes data formats, configuration ownership,
first-run imports, runtime downloads, or rollback behavior.
