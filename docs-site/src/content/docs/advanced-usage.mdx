...
Before upgrading the workspace to `0.1.7-alpha.1` or later, back up
`$DSH_HOME` (or the configured data directory) and review these data and
configuration changes:

- The official DeepSeek adapter no longer reads `protocol`. Remove that key
  from existing adapter settings and re-check the adapter's Messages API
  configuration before the first run.
- Session logs migrate to V4. The migration changes on-disk session data, so
  reverting to an older Nix generation restores program files but does not
  restore the old session format. Restore the backup or use a separate data
  directory when testing an older generation.
- The `settings.yaml` import is one-time: startup renames the source to
  `settings.yaml.imported` before writing the new settings and does not retry
  it after a later overwrite. In a managed profile, `cordis.patch.yml` is
  Nix-owned; use the declarative profile patch or choose `mutable` for runtime
  Settings changes.
- Voice input downloads its model on first use at runtime. This flake does not
  add a separate Nix model output or relocate the upstream plugin cache. Keep
  that cache in a writable, backed-up data location; offline use is available
  only after the model has been downloaded successfully for the same runtime
  user.
...
The build derives a `preset-web-subagents` plugin declaration from the shipped
`standard` declaration, removes `disabled` only from the listed rows, and adds
the declaration to the profile patch. The registry's `default` is set to
`web-subagents`. No separate preset directory is created. Bundle installation
and Agent Preset authorization are separate: installing a bundle does not
authorize its tools.
...
Two desktop applications are packaged:

- `dsh-desktop-official`: built from the desktop application in the pinned
  `dsh-workspace` release. Linux and Darwin.
- `dsh-desktop-unofficial`: built from
  [`anywhere-labs/deepseek-harness-desktop`](https://github.com/anywhere-labs/deepseek-harness-desktop).
  Linux and Darwin.

`dsh-desktop` is an alias that currently resolves to `dsh-desktop-unofficial`.
Reference the variant you want explicitly:

```sh
nix build .#dsh-desktop-official
nix run .#dsh-desktop-official
```

On Darwin the application is installed as
`Applications/DeepSeek Harness.app`, with `bin/dsh-desktop` as the launcher.

Both variants accept `dshHost`, the composed `dsh` behind the desktop runtime.
Point it at a preset or a custom composition to change the bundles the
application runs:

```nix
pkgs.dsh.dsh-desktop-official.override {
  dshHost = pkgs.dsh.presets.tui;
}
```
...
under `pkgs.dsh.helpers.*`, and package outputs such as `pkgs.dsh.dsh-desktop`
and `pkgs.dsh.dsh-desktop-official`. Flake `packages` expand the same scope, so
`nix build .#bundles.tui` and `nix run .#presets.web` work the same way.

### Helper Surface

Two kinds of helper are available to consumers outside this flake:

- `pkgs.dsh.helpers.buildBundle`, including `fromPnpmWorkspace`, `fromWorkspace`,
  and `validateDshBundle`: the bundle builders described below.
- `legacyPackages.<system>.importPnpmLock`, or `pkgs.dsh.importPnpmLock` from the
  overlay: the fetcher for bundles that vendor a `pnpm-lock.json`, plus
  `pnpm-workspace.json` when upstream ships one.

Every other helper in the scope, `pkgs.dsh.dshWorkspacePatchHook` included, is
an internal build implementation detail. `patchDshWorkspace kernel` reads the
workspace overrides from `DSH_WORKSPACE_OVERRIDES`, a path to that JSON;
`dsh-workspace` renders it from
`pnpmDeps.passthru.workspaceConfig.overrides`.

`importPnpmLock` takes `lockfileJson` and `workspaceJson`. It drops a `.` entry
from `workspaceJson.packages` when it writes `pnpm-workspace.yaml`, because pnpm
reads a bare `.` back as an empty package under the emitted `%YAML 1.1` header.
