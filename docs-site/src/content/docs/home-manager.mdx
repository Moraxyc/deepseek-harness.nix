...
- any setup: set `programs.dsh.package` explicitly and, when enabling the desktop,
  set `programs.dsh.desktop.package` too.
...
bundle separately. Home Manager manages the generated preset declaration in the
profile patch during activation; see [Advanced Usage](../advanced-usage/) for
the tool authorization example.
...
The module adds `pkgs.dsh.dsh-desktop-official` to `home.packages`;
