...
| `dsh-desktop-official`   | Desktop application from the upstream release  |
| `dsh-desktop-unofficial` | Desktop application from a third-party project |
...
`dsh-desktop` is an alias and currently resolves to `dsh-desktop-unofficial`.
See [Advanced Usage](../advanced-usage/) for the variant sources, the
supported platforms, and the `dshHost` override.
