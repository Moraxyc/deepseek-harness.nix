...
- `agentPreset`: an ID from `programs.dsh.agentPresets`; the build derives a
  `@deepseek-ai/dsh-agent-preset` declaration from the shipped source and adds
  it to the profile patch. The selected ID is set through
  `agent-preset-registry`; no `.agent-presets` directory is copied;
...
the profile. `source` selects one of the preset plugin declarations shipped by
the web bundle. `enableTools` removes `disabled` only from the listed rows in
the generated declaration; it does not install a provider bundle. See
[Advanced Usage](../advanced-usage/) for a complete subagent example.
...
Settings ownership follows the mode. The Settings UI writes the active
profile's `cordis.patch.yml`; in `managed` mode Nix owns that file, so declare
values through `profiles.<name>.patch` and expect the next activation or launch
to restore them. Choose `mutable` when runtime Settings or `dsh plugin` changes
must persist outside Nix. The upstream one-time migration imports an existing
`settings.yaml`, renames it to `settings.yaml.imported` before writing, and does
not retry that file after a later overwrite.
...
After importing the module and configuring the overlay described above, enable
the official desktop application independently:

```nix
programs.dsh.desktop.enable = true;
```

`desktop.enable` defaults to `false` and does not require `programs.dsh.enable`.
The module adds `pkgs.dsh.dsh-desktop-official` to `environment.systemPackages`;
override it with `programs.dsh.desktop.package`.
Select a declared profile with `desktop.profile`, independently of the CLI default:

```nix
programs.dsh = {
  desktop.enable = true;
  desktop.profile = "nix-desktop";
  profiles.desktop = {
    bundles = [ pkgs.dsh.bundles.notification ];
    mode = "managed";
  };
};
```

You can also reference `config.programs.dsh.profiles.desktop.materializedName`.
The module includes the web application dependencies and packages the profile's
bundles in the official desktop runtime. On command-line or application-menu
launch, the desktop seeds the selected profile, its referenced `agentPresets`, and
the home-level `patch` under `programs.dsh.home` before starting the host.
The CLI does not need to be enabled. `managed` mode restores declared configuration
on the next launch; `mutable` mode seeds it only once. Disable managed plugins in
Nix; desktop recovery does not rewrite a managed profile. Restart the desktop
after changing its configuration.

`desktop.profile` defaults to `null`, preserving the upstream `profiles/desktop`
behavior. This integration requires `desktop.package` to support the official
package's `dshHost` and `desktopProfile` arguments.
`programs.dsh.defaultProfile` still controls only the CLI.
