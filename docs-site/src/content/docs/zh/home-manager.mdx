...
- 任意方式：显式设置 `programs.dsh.package`；启用桌面时还需显式设置
  `programs.dsh.desktop.package`。
...
`agentPreset = "name"`，再单独列出 provider bundle。Home Manager 会在激活时管理
profile patch 中生成的 preset 声明；工具授权示例见[高级用法](../advanced-usage/)。
...
模块将 `pkgs.dsh.dsh-desktop-official` 加入 `home.packages`；
