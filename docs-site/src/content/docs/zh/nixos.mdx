...
- `agentPreset`：`programs.dsh.agentPresets` 中的 ID；构建时会从指定的上游
  preset 声明生成 `@deepseek-ai/dsh-agent-preset`，并加入 profile patch，
  再通过 `agent-preset-registry` 选择该 ID；不会复制 `.agent-presets` 目录；
...
Agent Preset 写在 `programs.dsh.agentPresets` 下，再由 profile 引用。`source`
选择 web bundle 随附的 preset 插件声明；`enableTools` 只会从生成声明中列出的
行删除 `disabled`，不会自动安装 provider bundle。子 agent 示例见[高级用法](../advanced-usage/)。
...
设置的归属由模式决定。Settings UI 会写入当前 profile 的
`cordis.patch.yml`；在 `managed` 模式下该文件由 Nix 所有，因此应通过
`profiles.<name>.patch` 声明，并预期下次激活或启动时恢复。需要让运行时
Settings 或 `dsh plugin` 的修改持久化时，请选择 `mutable`。上游的一次性迁移
会导入已有的 `settings.yaml`，在首次写入前将其改名为
`settings.yaml.imported`；文件之后被覆盖也不会再次自动导入。
...
导入模块并配置上述 overlay 后，可独立启用官方桌面应用：

```nix
programs.dsh.desktop.enable = true;
```

`desktop.enable` 默认关闭，不要求启用 `programs.dsh.enable`。
模块将 `pkgs.dsh.dsh-desktop-official` 加入 `environment.systemPackages`；
可通过 `programs.dsh.desktop.package` 覆盖安装包。
使用 `desktop.profile` 选择现有声明式 profile；桌面与 CLI 的默认选择独立：
...
也可引用 `config.programs.dsh.profiles.desktop.materializedName`。
模块自动加入 Web 应用依赖，并将 profile 的 bundles 打包进官方桌面 runtime。
从命令行或应用菜单启动时，桌面先按 `programs.dsh.home` 初始化选中的 profile、
其引用的 `agentPresets` 和 home 级 `patch`，再启动 host；不要求启用 CLI。
`managed` 模式在下次启动时恢复声明配置，`mutable` 模式只初始化一次。
管理模式下须在 Nix 中禁用插件，桌面的恢复操作不会改写该 profile。
修改配置后需重新启动桌面应用。

`desktop.profile` 默认为 `null`，此时保留上游 `profiles/desktop` 行为。
该集成要求 `desktop.package` 支持官方包的 `dshHost` 和 `desktopProfile` 参数。
`programs.dsh.defaultProfile` 仍只控制 CLI。
