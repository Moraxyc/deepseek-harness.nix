...
升级到 `0.1.7-alpha.1` 或更高版本前，请备份 `$DSH_HOME`（或配置的数据目录），
并检查以下数据与配置变化：

- 官方 DeepSeek adapter 不再读取 `protocol`。首次运行前，从现有 adapter 配置中
  删除该键，并重新检查 Messages API 的配置。
- Session 日志会迁移到 V4。迁移会改写磁盘上的会话数据；回退到旧 Nix generation
  只能恢复程序文件，不能恢复旧的会话格式。测试旧 generation 时，请恢复备份或
  使用独立的数据目录。
- `settings.yaml` 只导入一次：启动时会先把源文件改名为
  `settings.yaml.imported`，再写入新设置；之后文件被覆盖也不会再次导入。managed
  profile 中的 `cordis.patch.yml` 由 Nix 所有；需要使用声明式 profile patch，或选择
  `mutable` 保存运行时 Settings 修改。
- 语音输入会在首次使用时于运行时下载模型。本 flake 没有新增独立的 Nix 模型输出，
  也不会重定位上游插件的缓存目录。请保证该缓存位于可写且已备份的数据目录中；同一
  运行用户成功下载模型后，才可按缓存实现离线使用。
...
构建时会从随附的 `standard` 声明生成 `preset-web-subagents` 插件声明，
只从 `enableTools` 列出的行删除 `disabled`，并将声明加入 profile patch；
registry 的 `default` 会设为 `web-subagents`。不会创建独立的 preset 目录。
...
- `dsh-desktop-official`：由固定版本的 `dsh-workspace` 发布版中的桌面应用构建，
  支持 Linux 和 Darwin。
- `dsh-desktop-unofficial`：由
  [`anywhere-labs/deepseek-harness-desktop`](https://github.com/anywhere-labs/deepseek-harness-desktop)
  构建，支持 Linux 和 Darwin。

`dsh-desktop` 是别名，当前指向 `dsh-desktop-unofficial`。需要哪个变体就显式引用：
...
Darwin 下应用安装为 `Applications/DeepSeek Harness.app`，启动入口是
`bin/dsh-desktop`。

两个变体都接受 `dshHost`，即桌面运行时背后的组合后 `dsh`。把它指向 preset 或自定义
组合，就能改变应用运行的 bundle：
...
`pkgs.dsh.helpers.*` 下的公开 helper，以及 `pkgs.dsh.dsh-desktop`、
`pkgs.dsh.dsh-desktop-official` 等包输出。flake 的 `packages` 展开的是同一 scope，
因此 `nix build .#bundles.tui` 和
`nix run .#presets.web` 仍然可用。

### Helper 范围

本 flake 之外可以使用的 helper 有两类：

- `pkgs.dsh.helpers.buildBundle`，含 `fromPnpmWorkspace`、`fromWorkspace`
  和 `validateDshBundle`：下文用到的 Bundle 构建器。
- `legacyPackages.<system>.importPnpmLock`，overlay 下为
  `pkgs.dsh.importPnpmLock`：为通过 vendored `pnpm-lock.json`（上游提供时还有
  `pnpm-workspace.json`）取依赖的 Bundle 准备的 fetcher。

scope 里其余 helper，包括 `pkgs.dsh.dshWorkspacePatchHook`，属于构建的内部实现。
`patchDshWorkspace kernel` 从 `DSH_WORKSPACE_OVERRIDES` 读取 workspace
overrides，该变量指向对应 JSON 文件；`dsh-workspace` 用
`pnpmDeps.passthru.workspaceConfig.overrides` 渲染该文件。

`importPnpmLock` 接收 `lockfileJson` 和 `workspaceJson`。它写
`pnpm-workspace.yaml` 时会丢掉 `workspaceJson.packages` 里等于 `.` 的条目：
生成的 `%YAML 1.1` 头下，pnpm 会把单独的 `.` 读回成空 package。
